...
impl Solution {
  ...
  fn update(&mut self, start: i64, end: i64, index: i64, target: i64, val: i64) {
    ...
    let mid: i64 = (start + end) / 2;
    self.update(start, mid, index * 2, target, val);
    self.update(mid + 1, end, index * 2 + 1, target, val);
    self.tree[index as usize] = cmp::min(self.tree[(index * 2) as usize], self.tree[(index * 2 + 1) as usize]);
  }
...
fn main() {
  ...
  let n = gets.trim().parse::<i64>().unwrap();
  gets.clear();
  stdin.read_line(&mut gets).ok();
  let arr: Vec<i64> = gets.split_whitespace()
    .map(|x| x.parse::<i64>().unwrap())
    .collect();
  let mut solution: Solution = Solution::new(arr);
  gets.clear();
  stdin.read_line(&mut gets).ok();
  let m = gets.trim().parse::<i64>().unwrap();
  for _i in 0..m {
    gets.clear();
    stdin.read_line(&mut gets).ok();
    let split: Vec<i64> = gets.split_whitespace()
      ...
      .collect();
    if split[0] == 1 {
      solution.update(0, n-1, 1, split[1] - 1, split[2]);
    } else if split[0] == 2 {
      writeln!(stdout, "{}", solution.get_mins(0, n-1, 1, split[1] - 1, split[2] - 1)).ok();
    }
  }
}