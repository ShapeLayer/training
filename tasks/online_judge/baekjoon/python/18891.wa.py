...
class Party:
    def __init__(self, _id: int, name: str, district: Decimal, vote: Decimal):
        '''
        :param name: 정당 이름
...
def round(n: Decimal) -> int:
    return n + 1 if n - int(n) >= Decimal(.5) else n

def compute(p: int, v: int, av: Decimal, parties: list[Party]):
    def preprocess(p: int, v: int, av: Decimal, parties: list[Party]):
        '''
        전처리 단계
        봉쇄 조항 처리
        '''
    ...
    def compute_step_1(p: int, v: int, av: Decimal, parties: list[Party]):
        '''
    ...
    def compute_step_2(p: int, v: int, av: Decimal, parties: list[Party]):
        '''
        ...
        if linked_dist < 30:
            ...
            for party in parties:
                ...
                make_30 += party.p[2]
            for party in sorted(parties, key=lambda each: (each.p_rate[2], each.id), reverse=True):
                if not party.calc_target:
                    continue
                if make_30 >= 30:
                    break
    ...
    def compute_step_3(p: int, v: int, av: Decimal, parties: list[Party]):
        '''
        ...
        for party in sorted(parties, key=lambda each: (each.p_rate[3], each.id), reverse=True):
            if not party.calc_target:
                continue
            if total >= 17:
                break
...
if __name__ == '__main__':
    ...
    for i in range(p):
        ...
        parties.append(Party(i, name, Decimal(dist), Decimal(vote)))
        av += Decimal(vote)
